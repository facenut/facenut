...
def add_attendance_record(sno, event, checktime=None):
    ...
    sql = f"INSERT INTO attendance (sno, classno, event) select sno, classno, '{event}' FROM studentinfo WHERE sno = '{sno}' "
    
    dbms.RunSQL(sql)
    dbms.CloseQuery()
    dbms.DBClose()
    print(f"[추가] {checktime.time()} - {sno} : {event}")

def get_today_attendance_records(sno, date):
    """
    ...
    if dbflag == False :
        ...
        sql = f"""
            ...
            WHERE sno='{sno}' AND DATE(checktime)='{date}'
...
# =============================================================================
def auto_attendance(sno, current_dt, is_final=True):
    """
    ...
    records = get_today_attendance_records(sno, today)
    count = len(records)
    ...
    if count == 0:
        ...
        recs = get_today_attendance_records(sno, today)
